...
where t2.id=t1.trip";
	...
	for($i=1;$i< 32;$i++){
		...
		while(list($trip,$time)=each($trips)){
			...
			$q="select t1.id,t1.vehicle,t1.driver  from plan as t1 where  t1.customer='$customer' and t1.route='$route' and t1.trip='$trip' and t1.date='$date'  ";
			$ck=mysql_query($q); //print $q;
...
//>
			...
			print "<td onclick=\"$onclick\" onmouseover=this.style.background='#faa'; onmouseout=this.style.background=''; class='tdplan'><span class=$st>$vh<br>$dr</span></td>";
		}
		print "</tr>";
	}	
	print "</tbody></table>";
...
function editplan($plan){
	...
	$ck=mysql_query($q);
	list($plan,$vehicle,$driver)=mysql_fetch_array($ck);
	print "<table><form action=?action=plan-save&plan=$plan&customer=$customer&route=$route&trip=$trip&date=$date method=post>
...
<tr><td>".flddict('driver')."</td><td><select name=driver >".tboptions2('driver',$driver)."</select></td></tr>
...
if($action=='trip'){
	print "<table><form action=?action=$action method=get><input type=hidden name=action value=$action><tr><td> Select Customer <select name=customer onchange=this.form.submit();><option>...".tboptions2('customer',$customer)."</select></td><td>route</td><td><select name=route onchange=this.form.submit(); ><option>..".tboptions2('route',$route)."</select></td></tr></form></table>"; 
	$tb=$action;
	if(($customer)&&($route)){
...
if($action=='plan'){
	print "<table><form action=?action=$action method=get><input type=hidden name=action value=$action><tr><td> Select Customer <select name=customer onchange=this.form.submit();><option value=''>...".tboptions2('customer',$customer)."</select></td><td>route</td><td><select name=route onchange=this.form.submit(); ><option value=''>..".tboptions2('route',$route)."</select></td><td>Month</td><td>".monthnav2()."</td><td><input type=submit value=go></td></tr></form></table>"; 
	$tb=$action;
	if(($customer)&&($route)){
...
if($action=='plan-add'){
	...
	if(mysql_num_rows($ck)==0){
		$q=" insert into plan
		(customer,route,trip,date,in_out,time,plan_start,actual_start,status,plan_by,logs,vehicle,driver) values
		('$customer' ,'$route' ,'$trip' ,'$date' ,'$in_out' ,'$start_time' ,'$start_time','$start_time', 'plan','$_SESSION[user]','$now added by $_SESSION[user]\n','$vehicle','$driver' ) 
		";
